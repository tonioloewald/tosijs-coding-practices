...
| --- | --- | --- | --- | --- | --- |
| [tosijs](https://github.com/tonioloewald/tosijs) | path-based observant state | 1.7.9 | 1.7.9: `take()` transforms inside list templates fixed — descriptor moved onto the binding entry so cloned rows stop sharing one change-detection memo. **1.8.0-rc.1 in flight**: `enableAgentInterface()` agent surface (describe/read/write/observe/call, auto-registers WebMCP tools, ~11 KB gz, read-only by default), contracts at app/component/element level, `auditAccessibility()`, schematic renderer, Apache-2.0 relicense + 2.0 deprecations (see Migration.md). 1.7.8 concluded the `parts` saga (pre-hydration ownership capture, #20/#21); **1.7.6 & 1.7.7 deprecated** | [site](https://tosijs.net) · [changelog](https://github.com/tonioloewald/tosijs/blob/main/CHANGELOG.md) | 2026-08-17 |
...
| [tosijs-schema](https://github.com/tonioloewald/tosijs-schema) | type-by-example schema / validation | 1.5.1 | 1.5.1 (#3): `.optional` no longer leaks the internal `x-tjs-optional` marker into serialized schemas — the flag rides the builder object, so `.schema`/`describe()` stay clean regardless of chaining order. 1.5.0: fail-open validator fixes (GHSA due for ≤1.4.0) + `agentContract`/`checkExamples`/`$counterexamples` (the agent-surface contract seam, #2) | [changelog](https://github.com/tonioloewald/tosijs-schema/blob/main/CHANGELOG.md) | 2026-08-17 |
...
| [tosijs-timezone-picker](https://github.com/tonioloewald/tosijs-timezone-picker) | graphical timezone-picker web-component | 0.6.0 (⚠️ **npm still 0.5.3 — the 0.6.0 publish never landed**) | 0.6.0: linked to these practices; shipped as a blueprint + eagerly-hydrated component; bespoke demo → tosijs-ui doc site; fixed NaN quarter-hour offsets and value/timezone desync. npm's last publish is 0.5.3 (2026-03) — re-run the publish and confirm it lands (releasing.md) | [site](https://timezones.tosijs.net) · [changelog](https://github.com/tonioloewald/tosijs-timezone-picker/blob/main/CHANGELOG.md) | 2026-08-17 |

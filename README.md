...
| --- | --- | --- | --- | --- | --- |
| [tosijs](https://github.com/tonioloewald/tosijs) | path-based observant state | 1.10.2 (repo · tag · npm all agree) | **1.10.2 published** (09-14): redaction is not deletion — a secret link vanished from the agent map instead of appearing redacted, found by the tenth pre-release review; the rule kept is *never decide an element's fate by reading a record you just redacted* • **1.10.1** (09-06): the type-surface release — `withAttributes<A>` named, probes compile the built `.d.ts`, union-with-`any` trap caught ×4 • open: #39 (documented IIFE has never exposed a global) | [site](https://tosijs.net) · [changelog](https://github.com/tonioloewald/tosijs/blob/main/CHANGELOG.md) | 2026-09-18 |
| [tosijs-ui](https://github.com/tonioloewald/tosijs-ui) | web components + the doc-site system | ⚠️ npm `latest` **1.14.1** · `package.json` **1.14.1** · tag **v1.14.2** | ⚠️ **`v1.14.2` is tagged and the CHANGELOG tells 1.14.x users to upgrade to it, but it was never published** — npm's version list ends at 1.14.1, so the documented `import 'tosijs-ui/doc-browser'` fix is unreachable (sweep 09-18) • **1.14.1 published** — the 1.14.0 line lands: CodeMirror re-export surface (`tosijs-ui/codemirror`, #131), test-fence work (#142 RFC open) • recent line: silent-failure sweep (1.10.0) · WCAG AA palette + tunnel sharing (1.9.x) · build-failing audit gate (1.8.0) | [site](https://ui.tosijs.net) · [changelog](https://github.com/tonioloewald/tosijs-ui/blob/main/CHANGELOG.md) | 2026-09-18 |
| [tosijs-schema](https://github.com/tonioloewald/tosijs-schema) | type-by-example schema / validation | 1.10.2 (repo · tag · npm all agree) | 1.10.1 (#11): builder return types (`Obj`/`Str`/`Num`/`Arr` + the helpers inside them) were declared but never exported, so any library re-exporting a schema failed declaration emit with TS4023 — invisible to apps, fatal to publishers. 1.10.0 (#10, BREAKING): `agentContract` gained `affectedRoots()` + `{ unknownPath: 'refuse' }` so the fail-closed posture over UNCONTRACTED paths is expressible, and two construction fail-opens closed (a stray `schema` key built an accept-all gate; a non-boolean `strict` built a sampling gate). **Five review passes** — a regex path-canonicalizer was built during remediation and REVERTED before shipping (each pass found another bypass; the regex was quadratic on the gate's least-trusted input). The matcher's real limit is documented and pinned by tests instead; a tokenizer is the tracked fix. Prepublish smoke lane now packs the real tarball, installs it into a scratch consumer, and asserts the published `.d.ts` both typechecks AND survives a consumer's own declaration emit. 1.9.0 (#9), 1.8.0 (#8), 1.5.x: the fail-open sweep (GHSA-3qw7-pvr3-2gpq for ≤1.4.0) | [changelog](https://github.com/tonioloewald/tosijs-schema/blob/main/CHANGELOG.md) | 2026-09-18 |
| [tosijs-floorplan](https://github.com/tonioloewald/tosijs-floorplan) | agent-surface map → floorplan SVG (pure, dependency-free renderer; **née tosijs-schematic**, renamed 0.3.0 over the tosijs-schema near-collision) | 0.5.0 (repo · tag · npm all agree) | 0.4.0 • born of the haltija convergence (#1: two independently-built renderers merged); haltija becomes a producer at its 1.13; tosijs vendors the renderer; API keeps schematic-* names (multi-producer contract) | [changelog](https://github.com/tonioloewald/tosijs-floorplan/blob/main/CHANGELOG.md) | 2026-09-18 |
| [tjs-lang](https://github.com/tonioloewald/tjs-lang) | TS dialect: runtime validation, safety boundaries, sandboxed VM | 0.13.13 (repo · tag · npm all agree) | **0.13.13 published** (09-13) — `unsafe var` / `unsafe eval` now refused outright, a takeaway named up front because a patch normally cannot take anything away • 0.13.12: cleared the five defects of one class (passes misreading code that merely *mentions* their target syntax, #51) • 0.13.6: `defineAtom` defaults `effects:'io'` — capability-membrane security fix, shipped as a patch deliberately • 0.12.0: VM security review (SSRF, ReDoS) | [playground](https://tjs-platform.web.app) · [changelog](https://github.com/tonioloewald/tjs-lang/blob/main/CHANGELOG.md) | 2026-09-18 |
| [react-tosijs](https://github.com/tonioloewald/react-tosijs) | React bridge for tosijs state | 1.2.1 (repo · tag · npm all agree) | 1.1.0–1.2.1 published 2026-07-20 (uSES rewrite, off-ramp positioning); extras shared with ngx-tosijs (#3) | [site](https://react.tosijs.net) · [changelog](https://github.com/tonioloewald/react-tosijs/blob/main/CHANGELOG.md) | 2026-09-18 |
| [ngx-tosijs](https://github.com/tonioloewald/ngx-tosijs) | Angular bridge for tosijs state (signals, zoneless-first) | 0.9.1 (repo · tag · npm all agree) | born 2026-07-21: 0.9.0 name-stake + 0.9.1 same-day blocker fix; two-frameworks-one-state demo | [site](https://angular.tosijs.net) · [changelog](https://github.com/tonioloewald/ngx-tosijs/blob/main/CHANGELOG.md) | 2026-09-18 |
| [tosijs-3d](https://github.com/tonioloewald/tosijs-3d) | Babylon.js 3D components + doc site | 0.8.1 (repo · tag · npm all agree) | **0.8.1 published — plane landed** • 0.7.3 was breaking (GTA V biped controls) • pre-tag review caught a staged TLS **private key**, rewritten out while unpushed • 0.6–0.7 line: manta-recon first-adopter fixes, flight-model rework, systemic `frameDelta` fix | [site](https://3d.tosijs.net) · [changelog](https://github.com/tonioloewald/tosijs-3d/blob/main/CHANGELOG.md) | 2026-09-18 |
...
| tosijs-virta | task surface — where tasks live and die (`<tosi-virta>`); local `~/tosijs-virta` repo, no remote yet | 0.1.0 (local, hand-maintained) | **front-end work started 2026-09-16** — scaffold on `tosijs-ui/site`, M1/M2 core against an in-memory sync log, IndexedDB event-log tests; services blockers tracked as tosijs-platform#5–9 | design: `DESIGN.md` in-repo | 2026-09-17 |
| service-compris | the services layer (née tosijs-services): RBAC, write pipeline, role resolution — the universal endpoint's decision layer | npm `latest` **0.1.0** (name-staked 2026-09-16; hand-maintained row) | **working through blockers** (install system, agent auth, logical collections, blobs, snapshot — tosijs-platform#5–9) | [npm](https://www.npmjs.com/package/service-compris) | 2026-09-17 |
| [tosijs-product](https://github.com/tonioloewald/tosijs-product) | scroll-linked animation components | 0.8.0 (repo · tag · npm all agree) | 0.7.0 published; first-ever review ran here (report in `reviews/`) • 0.6.5 fixed 0.6.4's `files`-glob packaging blowout (13.7→2.8 MB) — **0.6.4 consumers should upgrade** • role: the ecosystem's scroll-story landing-page engine (tosijs-3d / ariosto showcases planned) | [site](https://product.tosijs.net) · [changelog](https://github.com/tonioloewald/tosijs-product/blob/main/CHANGELOG.md) | 2026-09-18 |
| [tosijs-timezone-picker](https://github.com/tonioloewald/tosijs-timezone-picker) | graphical timezone-picker web-component | npm `latest` **0.6.0** · `package.json` **0.6.0** · **untagged** | 0.6.0 published; **still untagged** (a `v0.6.0` tag at `f013750` is safe when wanted) • the weekly sweep caught `main` unpushed at 0.5.3; published tarball verified content-identical to HEAD | [site](https://timezones.tosijs.net) · [changelog](https://github.com/tonioloewald/tosijs-timezone-picker/blob/main/CHANGELOG.md) | 2026-09-18 |
| [haltija](https://github.com/tonioloewald/haltija) | browser control for AI agents | 1.12.9 (repo · tag · npm all agree) | **1.12.9 published (2026-09-12) — the four security fixes are live** (Babylon CDN pin+SRI, renderer-relay default-deny, local-origin WS gates, env-leak stop); Snowfox was pinned to 1.12.8 awaiting exactly this • #40 auth default-deny shipped in 1.12.8 • 1.12.6: Electron bump past two isolation bypasses, 285→14 packages • Dependabot enabled 2026-09-12 | [changelog](https://github.com/tonioloewald/haltija/blob/main/CHANGELOG.md) | 2026-09-18 |
| [wobbly](https://github.com/tonioloewald/wobbly) | Web-Worker parallel array ops (npm pkg **`wobbly-js`** — plain `wobbly` is someone else's) | ⚠️ npm `latest` **0.1.0** (`wobbly-js`) · `package.json` **0.6.0** · tag **v0.6.0** | ⚠️ repo 0.6.0 vs npm `wobbly-js` 0.1.0 — publish or record the hold • `gm-demo` proves the thesis end-to-end | [changelog](https://github.com/tonioloewald/wobbly/blob/main/CHANGELOG.md) | 2026-09-18 |
| [tosijs-editor](https://github.com/tonioloewald/tosijs-editor) | rich-text editor component — no contentEditable, no execCommand, no browser selection APIs (pkg `tosijs-styled-editor`) | ⚠️ npm `latest` **0.4.4** (`tosijs-styled-editor`) · `package.json` **0.4.5** · tag **v0.4.4** | back-burnered but heading to MVP in bursts • 2026-09-06: adopted `tosijs-ui/site` build, deleted the legacy jQuery tree, gained live-example browser tests (two bugs happy-dom structurally cannot see), `llms.txt`/CHANGELOG/audit gate, list formatting | [site](https://tonioloewald.github.io/tosijs-editor/) · [changelog](https://github.com/tonioloewald/tosijs-editor/blob/master/CHANGELOG.md) | 2026-09-18 |
| [lukko](https://github.com/tonioloewald/lukko) | capability-secured LLM agent middleware | `package.json` **0.1.0** · **untagged** | ⚠️ [#2](https://github.com/tonioloewald/lukko/issues/2) unfixed since 2026-08-23 (26 days): 2 critical / 11 high advisories, all from the `tjs-lang ^0.3.0` pin — `bun add tjs-lang@^0.13.13` deletes the whole subtree • private, nothing ships, but it installs on the dev machine | — | 2026-09-18 |
| [loewald-dot-com](https://github.com/tonioloewald/tosijs-platform) | Firebase full-stack platform — **becoming a load-bearing pillar: the ecosystem's service layer** (repo: `tosijs-platform`) | `package.json` **0.1.0** · tag **v0.1.0** | **very active** (14 commits 09-06→09-10): the "three legs" thesis — this repo is the service layer, and that is its product • access-lattice / authority redesign (root of trust = datastore access; write **schemas** over role checks; fail-open F1 now DENIES) • security fixes: unpublished posts were listed publicly, drafts advertised to crawlers • shadow-parity verification surfaced three bugs the never-run integration suite hid • `DECISIONS.md` ledger; SSR-is-public enforced structurally | — | 2026-09-18 |
| [kith-email](https://github.com/tonioloewald/kith-email) *(private)* | email client (Tauri desktop) | `package.json` **0.8.5** · **untagged** | quiet since 2026-07-11 | — | 2026-09-18 |
| [kilpi](https://github.com/tonioloewald/kilpi) | tiny HTML sanitizer for rich-text paste paths — denylist, zero deps, ~0.8 kB gzipped (pkg `tosijs-kilpi`); passes DOMPurify's published 223-fixture corpus with zero executable residue | 1.0.0 (repo · tag · npm all agree) | 2026-09-17: extracted from tosijs-editor after three adversarial review rounds hardened it; DOMPurify's corpus vendored as a hard publish gate — it caught a bypass 33 hand-written vectors missed | [security model](https://github.com/tonioloewald/kilpi/blob/master/SECURITY.md) | 2026-09-18 |

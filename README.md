...
| [haltija](https://github.com/tonioloewald/haltija) | browser control for AI agents | ⚠️ npm `latest` **1.12.9** · `package.json` **1.13.0-beta.1** · tag **v1.12.9** | **1.13.0-beta.1 published under `beta` (2026-09-26), the first release through the OIDC staged-publish workflow** (both transports by default, machine-level cert, loader fallback, testInBrowser; pre-minor review BLOCK → remediated → GO; known issue #44, origin gate due before 1.13.0) • `latest` is still 1.12.9, the four security fixes • Dependabot enabled 2026-09-12 | [changelog](https://github.com/tonioloewald/haltija/blob/main/CHANGELOG.md) | 2026-09-26 |
...
| [tosijs-editor](https://github.com/tonioloewald/tosijs-editor) | rich-text editor component — no contentEditable, no execCommand, no browser selection APIs (pkg `tosijs-styled-editor`) | 0.5.2 (repo · tag · npm all agree) | 2026-09-26 **0.5.1** (security): a `changeAuthor` display name containing `</style>` could break out of a raw-text element and become live HTML in `value` — patched, not minor'd, because `^0.5.0` reaches consumers and `^0.6.0` would not • **first release through OIDC + staged publishing**; the dry-run-before-tagging step meant the tag was cut once • **0.5.0**: tracked changes as content (`<tosi-ins>`/`<tosi-del>`, live typing/delete/cut/paste, accept-reject leaves no residue) + LLM proofreading round-trip, queryable spell check with a user dictionary, self-renumbering footnotes, word-sticky drag selection • sanitizer extracted as [`tosijs-kilpi`](https://www.npmjs.com/package/tosijs-kilpi) (18× smaller than DOMPurify, parity on its 223 fixtures) • structural change tracking (paragraph merges as blocks-out/blocks-in) on a branch for 0.6.0, through six review rounds | [site](https://tonioloewald.github.io/tosijs-editor/) · [changelog](https://github.com/tonioloewald/tosijs-editor/blob/master/CHANGELOG.md) | 2026-09-26 |

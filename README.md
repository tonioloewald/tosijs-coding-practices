...
| [tosijs-ui](https://github.com/tonioloewald/tosijs-ui) | web components + the doc-site system | 1.15.2 (repo · tag · npm all agree) | **1.15.2** — tjs-lang peer widened to `^0.13.1 || ^0.14.0`, verified against 0.14.0-rc.0 (#182; unblocks tjs-lang 0.14.0); shipped `bin/docs.ts` shim fixed (broken for installed consumers since June) • **1.15.1** — doc-test failures named one constant wrong line (frame now found by `sourceURL` tag); `toBeCloseTo` + `>=`/`<=` matchers (#142) • **1.15.0** — `:static` fences + `liveExamples: 'opt-in'`, Prism static highlighting, Firefox `<tosi-table>` row cap 10k → 520k | [site](https://ui.tosijs.net) · [changelog](https://github.com/tonioloewald/tosijs-ui/blob/main/CHANGELOG.md) | 2026-09-24 |

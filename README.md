...
| [tosijs-schema](https://github.com/tonioloewald/tosijs-schema) | type-by-example schema / validation | ⚠️ npm `latest` **1.10.1** · `package.json` **1.10.1** · tag **v1.10.0** | **1.10.0 published (2026-09-12)** — closes [#10](https://github.com/tonioloewald/tosijs-schema/issues/10), the ecosystem's **first external report** (`agentContract().check()` fail-open): `affectedRoots()` + `unknownPath:'refuse'` • 1.10.1 queued: #11, builder types never exported → TS4023 for any library re-exporting a schema • 1.9.0: `maxProperties` enforced everywhere | [changelog](https://github.com/tonioloewald/tosijs-schema/blob/main/CHANGELOG.md) | 2026-09-12 |

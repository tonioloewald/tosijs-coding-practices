...
- **Projects on the task board (virta) don't hand-write Activity** (owner, 2026-09-26): the
  board is their live scoreboard, so the cell just says so, and the public story is the
  linked changelog. Version and "As of" stay tool-generated for everyone.
...
| --- | --- | --- | --- | --- | --- |
| [tosijs](https://github.com/tonioloewald/tosijs) | path-based observant state | 1.10.3 (repo · tag · npm all agree) | **on the task board** — live status there; public history in the changelog | [site](https://tosijs.net) · [changelog](https://github.com/tonioloewald/tosijs/blob/main/CHANGELOG.md) | 2026-09-26 |
| [tosijs-ui](https://github.com/tonioloewald/tosijs-ui) | web components + the doc-site system | 1.15.5 (repo · tag · npm all agree) | **1.15.5** — `<tosi-select>` option shortcuts (#188); removed menus stopped firing shortcuts; `tosijs-ui/test-setup` published (#170); onboarded onto the virta board • **1.15.4** — `libraryBundle` (#169); tag colours (#173); first pre-tag dry run • **1.15.3** — first release via OIDC + staged publishing (#178) | [site](https://ui.tosijs.net) · [changelog](https://github.com/tonioloewald/tosijs-ui/blob/main/CHANGELOG.md) | 2026-09-26 |
| [tosijs-schema](https://github.com/tonioloewald/tosijs-schema) | type-by-example schema / validation | ⚠️ npm `latest` **1.10.2** · `package.json` **1.11.0** · tag **v1.10.2** | 1.10.1 (#11): builder return types (`Obj`/`Str`/`Num`/`Arr` + the helpers inside them) were declared but never exported, so any library re-exporting a schema failed declaration emit with TS4023 — invisible to apps, fatal to publishers. 1.10.0 (#10, BREAKING): `agentContract` gained `affectedRoots()` + `{ unknownPath: 'refuse' }` so the fail-closed posture over UNCONTRACTED paths is expressible, and two construction fail-opens closed (a stray `schema` key built an accept-all gate; a non-boolean `strict` built a sampling gate). **Five review passes** — a regex path-canonicalizer was built during remediation and REVERTED before shipping (each pass found another bypass; the regex was quadratic on the gate's least-trusted input). The matcher's real limit is documented and pinned by tests instead; a tokenizer is the tracked fix. Prepublish smoke lane now packs the real tarball, installs it into a scratch consumer, and asserts the published `.d.ts` both typechecks AND survives a consumer's own declaration emit. 1.9.0 (#9), 1.8.0 (#8), 1.5.x: the fail-open sweep (GHSA-3qw7-pvr3-2gpq for ≤1.4.0) | [changelog](https://github.com/tonioloewald/tosijs-schema/blob/main/CHANGELOG.md) | 2026-09-26 |
...
| [tosijs-3d](https://github.com/tonioloewald/tosijs-3d) | Babylon.js 3D components + doc site | 0.8.4 (repo · tag · npm all agree) | **on the task board** — live status there; public history in the changelog | [site](https://3d.tosijs.net) · [changelog](https://github.com/tonioloewald/tosijs-3d/blob/main/CHANGELOG.md) | 2026-09-26 |
...
| tosijs-virta | task surface — where tasks live and die (`<tosi-virta>`, CLI, MCP); private repo `github.com/tonioloewald/tosijs-virta`, deploys are the distribution | `package.json` **0.5.2** · **untagged** | **on the task board** — live status there; public history in the changelog | app: virta.tosijs.net · design: `DESIGN.md` in-repo | 2026-09-26 |
...
| [haltija](https://github.com/tonioloewald/haltija) | browser control for AI agents | ⚠️ npm `latest` **1.12.9** · `package.json` **1.13.0-beta.1** · tag **v1.12.9** | **on the task board** — live status there; public history in the changelog | [changelog](https://github.com/tonioloewald/haltija/blob/main/CHANGELOG.md) | 2026-09-26 |
...
| [tosijs-editor](https://github.com/tonioloewald/tosijs-editor) | rich-text editor component — no contentEditable, no execCommand, no browser selection APIs (pkg `tosijs-styled-editor`) | 0.5.2 (repo · tag · npm all agree) | **on the task board** — live status there; public history in the changelog | [site](https://tonioloewald.github.io/tosijs-editor/) · [changelog](https://github.com/tonioloewald/tosijs-editor/blob/master/CHANGELOG.md) | 2026-09-26 |

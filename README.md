...
| --- | --- | --- | --- | --- | --- |
| [tosijs](https://github.com/tonioloewald/tosijs) | path-based observant state | ⚠️ npm `latest` **1.10.1** · `package.json` **1.11.0** · tag **v1.10.1** | **1.11.0 pending publish** (`auditAccessibility()` rework on floorplan 0.4.0's exported rules — ends a written-down divergence) • **1.10.1** (09-06): the type-surface release — `withAttributes<A>` named, probes compile the built `.d.ts`, union-with-`any` trap caught ×4 • open: #39 (documented IIFE has never exposed a global) | [site](https://tosijs.net) · [changelog](https://github.com/tonioloewald/tosijs/blob/main/CHANGELOG.md) | 2026-09-13 |
| [tosijs-ui](https://github.com/tonioloewald/tosijs-ui) | web components + the doc-site system | 1.14.1 (repo · tag · npm all agree) | **1.14.1 published** — the 1.14.0 line lands: CodeMirror re-export surface (`tosijs-ui/codemirror`, #131), test-fence work (#142 RFC open) • recent line: silent-failure sweep (1.10.0) · WCAG AA palette + tunnel sharing (1.9.x) · build-failing audit gate (1.8.0) | [site](https://ui.tosijs.net) · [changelog](https://github.com/tonioloewald/tosijs-ui/blob/main/CHANGELOG.md) | 2026-09-13 |
| [tosijs-schema](https://github.com/tonioloewald/tosijs-schema) | type-by-example schema / validation | 1.10.1 (repo · tag · npm all agree) | 1.10.1 (#11): builder return types (`Obj`/`Str`/`Num`/`Arr` + the helpers inside them) were declared but never exported, so any library re-exporting a schema failed declaration emit with TS4023 — invisible to apps, fatal to publishers. 1.10.0 (#10, BREAKING): `agentContract` gained `affectedRoots()` + `{ unknownPath: 'refuse' }` so the fail-closed posture over UNCONTRACTED paths is expressible, and two construction fail-opens closed (a stray `schema` key built an accept-all gate; a non-boolean `strict` built a sampling gate). **Five review passes** — a regex path-canonicalizer was built during remediation and REVERTED before shipping (each pass found another bypass; the regex was quadratic on the gate's least-trusted input). The matcher's real limit is documented and pinned by tests instead; a tokenizer is the tracked fix. Prepublish smoke lane now packs the real tarball, installs it into a scratch consumer, and asserts the published `.d.ts` both typechecks AND survives a consumer's own declaration emit. 1.9.0 (#9), 1.8.0 (#8), 1.5.x: the fail-open sweep (GHSA-3qw7-pvr3-2gpq for ≤1.4.0) | [changelog](https://github.com/tonioloewald/tosijs-schema/blob/main/CHANGELOG.md) | 2026-09-13 |
| [tosijs-floorplan](https://github.com/tonioloewald/tosijs-floorplan) | agent-surface map → floorplan SVG (pure, dependency-free renderer; **née tosijs-schematic**, renamed 0.3.0 over the tosijs-schema near-collision) | 0.5.0 (repo · tag · npm all agree) | 0.4.0 • born of the haltija convergence (#1: two independently-built renderers merged); haltija becomes a producer at its 1.13; tosijs vendors the renderer; API keeps schematic-* names (multi-producer contract) | [changelog](https://github.com/tonioloewald/tosijs-floorplan/blob/main/CHANGELOG.md) | 2026-09-13 |
| [tjs-lang](https://github.com/tonioloewald/tjs-lang) | TS dialect: runtime validation, safety boundaries, sandboxed VM | ⚠️ npm `latest` **0.13.12** · `package.json` **0.13.13** · tag **v0.13.12** | **0.13.12 published** — cleared the five defects of one class (passes misreading code that merely *mentions* their target syntax, #51) • 0.13.6: `defineAtom` defaults `effects:'io'` — capability-membrane security fix, shipped as a patch deliberately • 0.12.0: VM security review (SSRF, ReDoS) | [playground](https://tjs-platform.web.app) · [changelog](https://github.com/tonioloewald/tjs-lang/blob/main/CHANGELOG.md) | 2026-09-13 |
| [react-tosijs](https://github.com/tonioloewald/react-tosijs) | React bridge for tosijs state | 1.2.1 (repo · tag · npm all agree) | 1.1.0–1.2.1 published 2026-07-20 (uSES rewrite, off-ramp positioning); extras shared with ngx-tosijs (#3) | [site](https://react.tosijs.net) · [changelog](https://github.com/tonioloewald/react-tosijs/blob/main/CHANGELOG.md) | 2026-09-13 |
| [ngx-tosijs](https://github.com/tonioloewald/ngx-tosijs) | Angular bridge for tosijs state (signals, zoneless-first) | 0.9.1 (repo · tag · npm all agree) | born 2026-07-21: 0.9.0 name-stake + 0.9.1 same-day blocker fix; two-frameworks-one-state demo | [site](https://angular.tosijs.net) · [changelog](https://github.com/tonioloewald/ngx-tosijs/blob/main/CHANGELOG.md) | 2026-09-13 |
| [tosijs-3d](https://github.com/tonioloewald/tosijs-3d) | Babylon.js 3D components + doc site | 0.8.1 (repo · tag · npm all agree) | **0.8.1 published — plane landed** • 0.7.3 was breaking (GTA V biped controls) • pre-tag review caught a staged TLS **private key**, rewritten out while unpushed • 0.6–0.7 line: manta-recon first-adopter fixes, flight-model rework, systemic `frameDelta` fix | [site](https://3d.tosijs.net) · [changelog](https://github.com/tonioloewald/tosijs-3d/blob/main/CHANGELOG.md) | 2026-09-13 |

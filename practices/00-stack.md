...
- **Leaf project — no dependents, not "no dependencies."** A leaf is a project nothing else
  (currently) consumes: it sits at the downstream edge of the graph and ships to *users*, not to
  other projects. It usually has plenty of *dependencies* (falling-forward consumes tosijs-ui);
  the zero-dependency pole is the opposite end (tosijs, wobbly — nothing upstream, everything
  downstream). Why it matters: the practices that protect *consumers* — semver discipline,
  release gates, API-seam caution, pre-release-review depth — scale with dependents and relax at
  a leaf. But agent-facing docs (`llms.txt`, `CHANGELOG.md` — [`development.md`](development.md))
  attach to every *product*, leaf apps included, because agents consume deployed apps too;
  falling-forward escapes those as *prose*, not as a leaf. "Currently" is load-bearing: leaf
  status is a fact, not a vow — the day something consumes a leaf, the consumer-protecting
  practices attach. — seen in: falling-forward; leaf apps: kith-email, lukko, ariosto,
  loewald-dot-com
...
- **Nearly all** — **no CI**; local quality gates + a "Landing the Plane" push discipline substitute for it (see [`releasing.md`](releasing.md)). Exceptions: haltija (3 GitHub Actions incl. Docs-Drift), tosijs-ui (GitHub Actions: `tsc` + unit lane + a Chromium E2E job; Firefox/WebKit only in the manual local lane), **tosijs** (`.github/workflows/ci.yml` since e12d641: a `unit` lane and a Playwright `e2e` lane, `main` only — so branch work is gated locally, not by CI — seen in: tosijs 1.8.0), tjs-lang (**two** gates with deliberately different coverage: GitHub Actions runs `test:fast` — no LLM, benchmarks or audit — while `.githooks/pre-push` runs the FULL suite on tag pushes only, where a human is present to warm the model server).
